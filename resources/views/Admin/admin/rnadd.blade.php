	...
	<form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
					...
					<dd>
						...
						@foreach($auth as $v)
							<dt style="width:160px">
								<label class="">
									<input type="checkbox" value="{{$v->id}}" name="nids[]" id="user-Character-0-0" {{in_array($v->id,$nids)?'checked':''}}>
									{{$v->name}}</label>
							</dt>
						@endforeach
						</dl>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
				<button type="submit" class="btn btn-success radius" id="admin-role-save"><i class="icon-ok"></i>分配权限</button>
			</div>
		</div>
	</form>
</article>
...
$(function(){
	...
	$("#form-admin-role-add").validate({
		...
		submitHandler:function(form){
			str=$('#form-admin-role-add').serialize();
			// $(form).ajaxSubmit();
			// var index = parent.layer.getFrameIndex(window.name);
			// parent.layer.close(index);
			$.post('/admin/adminsetauth/'+{{$data->admin_level}},{str:str},function(data){
				if(data==1){
					...
					layer.msg('权限不足，添加失败!',{icon:1,time:1000});
				}
			});
		}
	});
