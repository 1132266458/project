	...
	<table class="table table-border table-bordered table-bg">
		...
		@foreach($data as $k=>$v)
			<tr class="text-c">
				...
				<td>{{$v->admin_id}}</td>
				<td><a href="javascript:" onclick="admin_info('管理员信息','/admin/admininfo/{{$v->admin_id}}','450','400')">{{$v->admin_name}}</a></td>
				<td>
				{{$level[$k]}}
				</td>
				<td>{{date('Y-m-d h:m:s',$v->admin_addtime)}}</td>
				@if($v->admin_statue==0)
				...
				<td class="td-manage">
				@if($v->admin_statue==0)
			        <a onclick="status(this,{{$v->admin_id}},1)" href="javascript:;" title="禁用"  style="text-decoration:none"><i class="Hui-iconfont">&#xe631;</i></a> 
			      @else
			        <a style="text-decoration:none" onclick="status(this,{{$v->admin_id}},0)" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe615;</i></a>
			      @endif
				 <a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑','/admin/adminlist/{{$v->admin_id}}/edit','700','400')" class="ml-5" style="text-decoration:none"> <i class="Hui-iconfont">&#xe63c;</i></a>
				 <a title="信息编辑" href="javascript:;" onclick="admin_edit('管理员信息编辑','/admin/admininfo_edit/{{$v->admin_id}}','700','400')" class="ml-5" style="text-decoration:none"> <i class="Hui-iconfont">&#xe62e;</i></a>

				 <a title="删除" href="javascript:;" onclick="admin_del(this,{{$v->admin_id}})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
		@endforeach
		</tbody>
	</table>
