	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td><?php echo e($v->dev['order_sn']); ?></td>
				<td><?php echo e($v->dev['goods_name']); ?></td>
				<td>
				<?php if($v->reason_level==1): ?>
				货不对板
				<?php elseif($v->reason_level==2): ?>
				货物质量有问题
				<?php elseif($v->reason_level==3): ?>
				发错货了
				<?php endif; ?>
				</td>
				<td><?php echo e($v->reason); ?></td>
				<td><?php echo e(date('Y-m-d h:m:s',$v->return_addtime)); ?></td>
				<td class="td-status">
				<?php if($v->return_state==0): ?>
				<span class="label label-warning radius">等待处理</span>
				<?php elseif($v->return_state==1): ?>
				<span class="label label-success radius">退款成功</span>
				<?php elseif($v->return_state==2): ?>
				<span class="label label-danger radius">拒绝退款</span>
				<?php endif; ?>
				</td>
				<td>
				<?php if($v->return_state==0): ?>
				<a title="确认退款" href="javascript:;" onclick="oktui(this,<?php echo e($v->id); ?>)" class="btn btn-success radius size-S" style="text-decoration:none">确认退款</a><a title="拒绝退款" href="javascript:;" onclick="notui(this,<?php echo e($v->id); ?>)" class="btn btn-danger radius size-S" style="text-decoration:none">拒绝退款</a>
				<?php endif; ?>
				</td>
			</tr>
